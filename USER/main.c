...
void flash_judge_reg()
{
    ...
    readPackedMessageFromFlash(&temp_modbus_reg,sizeof(T_MODBUS_REG));
...
#ifdef TEMP_N55_P120
		pwm_value=(temp+5500)*0.5714;
#endif
#ifdef TEMP_N55_P85
		pwm_value=(temp+5500)*0.714;
#endif
...
//	set_humi_pwm(before_humi);
    ...
    while(1)
	{
        ...
        g_temperature=DS18B20_Get_Temp();
				if(g_temperature==-10000){  // explain value have not found
				  	before_temperature=-10000;
						continue;
				}
				
        g_temperature= g_temperature+gt_modbus_reg.temperature_correct_reg; //增加校对值
		
				if(before_temperature==-10000){  //表示第一次
						before_temperature=g_temperature;
				}
				//如果第一次那就跳动一样  一次跳动在5°范围内
				if(((g_temperature-before_temperature)>-500)&&((g_temperature-before_temperature)<500)){
						gt_modbus_reg.temperature_reg=g_temperature;
						err_num=0;
				}
				else{ //表示数据跳动太大，有可能是错误
						err_num++;
						if(err_num>10){
								before_temperature=-10000;
								err_num=0;
				...
				before_temperature=g_temperature;
        if(gt_modbus_reg.temperature_reg>20000){
            gt_modbus_reg.temperature_reg=20000;
        }
        else if(gt_modbus_reg.temperature_reg<-6000){
            gt_modbus_reg.temperature_reg=-6000;
        }
